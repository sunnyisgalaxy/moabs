...
namespace stan {
  ...
  namespace common {
    ...
    int command(int argc, const char* argv[], std::istringstream & ifs) {
...
//      data_stream.close();
      stan::io::dump data_var_context(ifs);
      ...
      //dsun: remove generation of output file
//      if (output_file != "") {
...
//      }
      ...
      //dsun: remove model output
      //parser.print(&std::cout);

      //std::cout << "dsun" << std::endl;
      //std::cout << std::endl;
...
//      }
      ...
      if (parser.arg("method")->arg("sample")) {
        ...
        //comment the following to remove output
//        std::cout << std::endl;
